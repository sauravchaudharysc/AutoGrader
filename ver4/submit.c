...
//Arguments: socket fd, file name (can include path)
{
    ...
    char dummy[10];
    	recv(sockfd,dummy,sizeof(dummy),0);
...
int main(int argc, char *argv[])
{
    if (argc !=5)
    {
        perror("Usage: ./submit  <serverIP> <port> <requestType> <sourceCodeFileTobeGraded>\n");
        return -1;
    }
...
// setup the server side variables
    ...
    if(strcmp(requestType,"new")!=0){
    	int n=send(sockfd,"REQUEST\n",sizeof("REQUEST\n"),0);
    	char dummy[10];
    	recv(sockfd,dummy,sizeof(dummy),0);
    	int m=send(sockfd,argv[4],sizeof(argv[4]),0);
    	char msg[500];
